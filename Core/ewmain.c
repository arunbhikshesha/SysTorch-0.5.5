...
#include "stm32g4xx_hal.h"   //XXXX g4 statt g0
...
#if EW_USE_EXTERNAL_FLASH == 1   //XXXX SysBrenner kein ext. FLASH
    /* initialize flash reader for accessing flash that is not in CPU address range */
  EwPrint( "Register Flash Reader...                     ");
  CHECK_HANDLE( EwBspFlashReaderInit( (void*)EW_EXT_FLASH_ADDR, EW_EXT_FLASH_SIZE ));
#endif
...
void EwDone( void )
{
#if 0 //XXXX EwDone() wird nicht verwendet
...
int EwProcess( void )
{
  ...
  events = IOProcessKeys(RootObject);
  timers = EwProcessTimers();
  param_process = process_param_update();

  /* refresh the screen, if something has changed and draw its content */
  if ( signals | events | timers | param_process)
  {
...
*   Alternatively, the joystick can be used to provide navigation key events.
*
* ARGUMENTS:
*   None
*
...
XEnum EwGetKeyCommand( void )
{
  ...
  #else
    switch ( EwBspInOutGetJoystickState())
    {
      case EW_JOYSTICK_CENTER : return CoreKeyCodeOk;
      case EW_JOYSTICK_RIGHT  : return CoreKeyCodeRight;
      case EW_JOYSTICK_LEFT   : return CoreKeyCodeLeft;
      case EW_JOYSTICK_DOWN   : return CoreKeyCodeDown;
      case EW_JOYSTICK_UP     : return CoreKeyCodeUp;
    }
  #endif
